...
public class HealthUI : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (_pm.TryGetState(_localActor, out PlayerInfoData d))
        {
            ...
            if (!Mathf.Approximately(slider.maxValue, maxHp))
            {
                slider.maxValue = Mathf.Max(1, maxHp);
                _display = Mathf.Clamp(_display, 0, slider.maxValue);
            }

            float target = Mathf.Clamp(d.hp, 0, (int)slider.maxValue);

            _display = (smoothSpeed > 0f)
                ? Mathf.MoveTowards(_display, target, smoothSpeed * Time.deltaTime)
                : target;
    ...
    private void PrimeFromCache()
    {
        ...
        {
            slider.maxValue = Mathf.Max(1, maxHp);
            _display = Mathf.Clamp(d.hp, 0, (int)slider.maxValue);
            slider.SetValueWithoutNotify(_display);
